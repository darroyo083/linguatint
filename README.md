...
**Early beta.** Likely buggy on edge cases. Detection is heuristic — expect misses on short or ambiguous text.
...
1. **Parentheses** — text inside `(...)` is Spanish.
...
3. **Word-level** — centered 5-word window using: exclusive characters (ß, ñ, ö, ü, é, í…), 500+ word dictionary per language, capital letter (German noun), suffixes (-ción, -dad, -mente, -istmo) and character patterns (sch, cht, ung; que, ión, miento, cción).

Glued words like `einemKind` are split automatically if both halves are in the dictionary.
...
```sh
node test-detector.js   # 42 detection tests
node test-wordlevel.js  # 8 segmentation tests
```
